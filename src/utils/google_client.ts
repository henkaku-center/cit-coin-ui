...
const creds = JSON.parse(
  Buffer.from(process.env.GOOGLE_SERVICE_ACCOUNT_CREDENTIALS || '', 'base64').toString('ascii'),
);
...
export const firebase_client = {
  getQuests: () =>
    new Promise<TQuestStorage>((resolve, reject) => {
      get(child(ref(firebase_db), `sheets/${_quest_path}`))
        .then((snapshot) => {
  ...
  setQuests: (data: Quest[], sheetId?: string) =>
    new Promise((resolve, reject) => {
      ...
      set(ref(firebase_db, `sheets/${_quest_path}`), quest)
        .then((resp) => {
          resolve(resp);
        })
        .catch((err) => {
