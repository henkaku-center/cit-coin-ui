import {
  ...
  Spinner,
  Stack, useDisclosure,
  Modal, ModalOverlay, ModalContent, ModalBody, ModalHeader, ModalCloseButton, ModalFooter, useToast,
    ...
    <>
      <Stack mb={5} direction={'row'}>
        {sheets.length > 0 && <Select
          ...
          {sheets.map((sheet, index) => <option key={index} value={sheet}>
            {sheet} {sheet === currentSheet?.sheetId && `( ${t('quest.CURRENT_SHEET')} )`}
          </option>)}
        </Select>}
        {currentSheet?.sheetId === activeSheet && <Button
          colorScheme={'orange'}
          isLoading={sheetsLoading}
          isDisabled={currentSheet?.sheetId !== activeSheet}
          onClick={onOpen}
          leftIcon={<FaSync/>}
        >{t('quest.UPDATE_CHANGES')}</Button>
        }
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button isLoading={sheetsLoading} colorScheme='blue' mr={3} onClick={(event) => {
              event.preventDefault();
              setSheetsLoading(true);
              sheetActions.setActiveSheet(activeSheet).then((resp) => {
                toast({
            ...
            <Button isDisabled={sheetsLoading} colorScheme={'red'} onClick={onClose}>{t('quest.modal.NO')}</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
    </>
